...
// Make a request for a users with a given ID
...
```js
{
  ...
  paramsSerializer: {
    ...
    // Custom serializer function for the entire parameter. Allows users to mimic pre 1.x behaviour.
...
| config   | An axios config object with specific instance configurations defined by the users from when the request was made |
...
| ERR_CANCELED              | Feature or method is canceled explicitly by the users using an AbortSignal (or a CancelToken).                                                                                                                                                                                                                                                                                                  |
...
source.cancel('Operation canceled by the users.');
```
...
- `visitor: Function` - users-defined visitor function that will be called recursively to serialize the data object
to a `FormData` object by following custom rules.

- `dots: boolean = false` - use dot notation instead of brackets to serialize arrays and objects;

- `metaTokens: boolean = true` - add the special ending (e.g `users{}: '{"name": "John"}'`) in the FormData key.
...
The option can also accept a users-defined function that determines whether the value should be overwritten or not.
...
let users: User = null;
try {
  const { data } = await axios.get('/users?ID=12345');
